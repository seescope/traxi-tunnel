...
use mio::{Io, EventSet, PollOpt};
use libc::{c_int};
use jni_sys::{JNIEnv, jobject, jstring, jmethodID};
...
impl Environment for AndroidEnvironment {
    ...
    fn get_package_name(&mut self, package_id: usize) -> String {
        unsafe {
            self.package_id_map.entry(package_id).or_insert({
                let other_jre = **self.jre;
                let package_id = package_id as c_int;
                let method = get_method_id(self.jre, self.thiz, "getApplicationName", "(I)Ljava/lang/String;");

                // Call the method.
                let package_name_jstring = (other_jre.CallObjectMethod)(self.jre, self.thiz, method,
                                                                 package_id) as jstring;
...
fn get_method_id(env: *mut JNIEnv, thiz: jobject, method_name: &str, method_signature: &str) -> jmethodID {
    unsafe {
        let other_jre = **env;
...
fn jstring_to_str(jstring: jstring, jre: *mut JNIEnv) -> String {
    unsafe {
        debug!("jstring_to_str| Attempting to convert jstring {:?}", jstring);
        let other_jre =  **jre;
        let mut is_copy = 0u8;
        ...
        match cstring.to_str() {
            ...
            Err(e)      => {
                error!("jstring_to_str| Error converting {:?} to jstring: {:?}", cstring, e);
                panic!();
            }
        }
    }
...
fn report_installed(uuid: String) {
    spawn(move || {
        ...
        firebase_connector.report_installed().map_err(|e| error!("{:?}", e));
    });
}


#[allow(unused_must_use)]
fn start_tunnel(environment: AndroidEnvironment, fd: c_int, uuid: String, file_path: String) -> Result<()> {
    // Build the tunnel.
    ...
    info!("START_TUNNEL| Removing existing ipc file at: {:?}", ipc_path);
...
pub extern fn Java_com_traxichildapp_TraxiVPNService_start(jre: *mut JNIEnv, thiz: jobject, fd: c_int, path: jstring, uuid: jstring) -> c_int {
    ...
    let environment = AndroidEnvironment { jre: jre, thiz: thiz, package_id_map: HashMap::new(), uuid: uuid.clone(), file_path: file_path.clone() };
